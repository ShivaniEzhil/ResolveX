...
export const AdminContainer: React.FC = () => {
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      ...
      case "users":
        return <AdminUsers onNavigate={(id) => setActiveTab(id)} />;
      case "analytics":
        return <AdminAnalytics onNavigate={(id) => setActiveTab(id)} />;
      case "audit-logs":
        return <AdminAuditLogs onNavigate={(id) => setActiveTab(id)} />;
      case "notifications":
        return <AdminNotifications onNavigate={(id) => setActiveTab(id)} />;
      case "dashboard":
      default:
        return (
          <AdminDashboard
            onNavigateTab={(id) => setActiveTab(id)}
            onSelectComplaint={(c: any) => handleSelectComplaint(c as ComplaintItem)}
          />
        );
    }
  };
